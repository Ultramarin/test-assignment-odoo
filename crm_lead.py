...
from odoo.addons.crm.models.secret import TOKEN, CHAT_ID
...
class Lead(models.Model):
    ...
    def _onchange_stage_id(self):
        ...
        self.update(values)
#Зміни тестове завдання
        stage = self.env['crm.stage'].browse(self.stage_id.id)#Отримання прикріпленої колонки колонки

        if stage.name == 'Завершена успешно':
            date_message = date.today()
            message = f"{self.name} \n" \
                      f" Дата {date_message} \n Дата завершення opportunity {self.date_deadline if self.date_deadline else 'Не вказано'}"#збираємо повідомлення
            bot = Bot(TOKEN)#підключення боту
            bot.send_message(CHAT_ID, message)#відправка повідомлення
    ...
    def write(self, vals):
        ...
        data = {"name":self.name, "stage_id":self.stage_id, }
